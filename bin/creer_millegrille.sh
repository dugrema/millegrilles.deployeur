...
creer_repertoires() {
  ...
  REP_MILLEGRILLES_PKI_DEPLOYEUR=$REP_MILLEGRILLE/$MILLEGRILLES_DEPLOYEUR_SECRETS

  # Creer repertoire de la millegrille (/var/opt/$IDMG)
  sudo mkdir -p $REP_MILLEGRILLES_PKI_DEPLOYEUR
  ...
  sudo chown $MILLEGRILLES_USER_DEPLOYEUR:$MILLEGRILLES_GROUP $REP_MILLEGRILLES_PKI_DEPLOYEUR
  ...
  sudo chmod 2700 $REP_MILLEGRILLES_PKI_DEPLOYEUR
}
